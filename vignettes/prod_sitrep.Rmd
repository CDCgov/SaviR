...
df <- onetable %>%
  ...
  calc_add_risk() %>%
  left_join(get_vax(), by = c("iso3code" = "iso_code", "date"))
```
...
burden_map_global <- map_burden(df %>%
  filter(iso3code != "CHN" | source != "WHO") %>%
...
global_vax_coverage <- df %>%
  filter(date <= sunday_date) %>%
  filter(iso3code != "CHN" | source != "WHO") %>%
  mutate(state_region = ifelse(iso3code == "USA", "US", state_region)) %>%
