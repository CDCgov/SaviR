...
get_owid_meta <- function(url = "https://ourworldindata.org/coronavirus-testing") {
  ...
  data_cleaned <- data_cleaned %>% 
                  relocate(test.def, test_definition, 
...
get_testing_long <- function(owid_all_source = "https://raw.githubusercontent.com/owid/covid-19-data/master/public/data/owid-covid-data.csv", 
 ...
 testing_OWID <-
               ...
               filter(!(Entity %in% c("Poland - samples tested", "Italy - people tested", 
 ...
 n_test_iso <- testing_OWID %>% count(iso_code, date) %>% filter(n > 1)
 ...
 full_OWID <- 
             ...
             select(iso_code, location, date, population, total_cases, new_cases, new_cases_smoothed,
                         tests_units)
 ...
 find_longdata <- left_join(find_data2, find_meta, by = "iso_code") %>%
   ...
   mutate(population = pop_100k * 100000,
          cumtest_available = T) %>%
   select(iso_code, date, population, total_tests_original, new_tests_original,
...
get_preferred_tests14 <- function(test_long, last_X_days = 14, analysis_date = NULL) {
  ...
  owid_df <- test_long %>% filter(data_source == "OWID") %>% filter(date <= analysis_date)
  find_df <- test_long %>% filter(data_source == "FIND") %>% filter(date <= analysis_date)
  ...
  all_recent_final <- all_recent %>%
    ...
    relocate(iso_code, preferred_source, preferred_recent_date, new_tests_daily14:new_tests_daily14_1k_prev,
             starts_with("flag_"))
...
get_preferred_testpos7 <- function(test_long, last_X_days = 14, analysis_date = NULL) {
  ...
  owid_df <- test_long %>% 
    ...
    # Correct FLAG for 8 countries which only report new tests daily, not cumulative
    mutate(FLAG_increase_tests_7day = replace(FLAG_increase_tests_7day, is.na(FLAG_increase_tests_7day), 0),
  ...
  all_recent <- 
    full_join(
      owid_recent %>%
        select(iso_code, owid_date = date, 
      ...
      find_recent %>%
        select(iso_code, find_date = date, 
  ...
  all_recent_final <- all_recent %>%
    ...
    relocate(iso_code, preferred_source, preferred_recent_date, positive_rate, positive_rate_prev,
...
preferred_long_locf <- preferred_long %>%
                  group_by(iso_code) %>% 
                  arrange(date) %>% 
                  mutate(new_tests_smoothed_per_thousand    = zoo::na.locf(new_tests_daily7_per_1k, na.rm=F, maxgap=14),
