...
plot_vaxcoverage <- function(df, type = c("People", "Fully", "Booster", "Pop18"), by_cat = "US Department of State Region", plot = c("Vaccination", "Population")) {
  type <- match.arg(type)
  plot <- match.arg(plot)
  ...
  } else if (by_cat == "US Department of State Region") {
    col_master <- state_aes
